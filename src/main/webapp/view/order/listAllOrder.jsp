...
<body bgcolor='lightpink'>
		...
		<%@ include file="page1.file" %>
			<c:forEach var="itemordervo" items="${orderVOlist}" begin="<%=pageIndex%>"
				...
				<tr>
					...
					<td class="orderState">${itemordervo.orderState}</td>
					...
					<td>
						<form method="post" action="<%=request.getContextPath()%>/view/order/order.tw"
							...
							<input type="hidden" name="orderNo" value="${itemordervo.orderNo}">
							<input type="hidden" name="test1" value="delete">
						</form>
					</td>
				</tr>
			</c:forEach>
	...
	<script>
		var orderStateElements = document.querySelectorAll('.orderState');
		orderStateElements.forEach(function(element) {
			var orderState = element.textContent.trim(); // 获取并去除首尾空格
			if (orderState === '0') {
